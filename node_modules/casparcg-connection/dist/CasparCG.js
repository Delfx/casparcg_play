...
class CasparCG extends events_1.EventEmitter {
    ...
    loadbg(channel, layer = NaN, clip, loop, transition, transitionDurationOrMaskFile, transitionEasingOrStingDuration, transitionDirectionOrOverlay, seek, length, filter, auto, channelLayout) {
        return this.do(new AMCP_1.AMCP.LoadbgCommand(Object.assign({ channel: channel, layer: layer, clip: clip, loop: loop, transition: transition }, this._createTransitionOptionsObject(transition, transitionDurationOrMaskFile, transitionEasingOrStingDuration, transitionDirectionOrOverlay), { seek: seek, length: length, filter: filter, auto: auto, channelLayout })));
    }
    loadbgAuto(channel, layer = NaN, clip, loop, transition, transitionDurationOrMaskFile, transitionEasingOrStingDuration, transitionDirectionOrOverlay, seek, length, filter, channelLayout) {
    ...
    load(channel, layer = NaN, clip, loop, transition, transitionDurationOrMaskFile, transitionEasingOrStingDuration, transitionDirectionOrOverlay, seek, length, filter, channelLayout) {
        return this.do(new AMCP_1.AMCP.LoadCommand(Object.assign({ channel: channel, layer: layer, clip: clip, loop: loop, transition: transition }, this._createTransitionOptionsObject(transition, transitionDurationOrMaskFile, transitionEasingOrStingDuration, transitionDirectionOrOverlay), { seek: seek, length: length, filter: filter, channelLayout })));
    }
    play(channel, layer = NaN, clip, loop, transition, transitionDurationOrMaskFile, transitionEasingOrStingDuration, transitionDirectionOrOverlay, seek, length, filter, channelLayout) {
        return this.do(new AMCP_1.AMCP.PlayCommand(Object.assign({ channel, layer, clip, loop, transition }, this._createTransitionOptionsObject(transition, transitionDurationOrMaskFile, transitionEasingOrStingDuration, transitionDirectionOrOverlay), { seek, length, filter, channelLayout })));
    }
    /**
    ...
    playDecklink(channel, layer = NaN, device, transition, transitionDurationOrMaskFile, transitionEasingOrStingDuration, transitionDirectionOrOverlay, length, filter, format, channelLayout) {
        return this.do(new AMCP_1.AMCP.PlayDecklinkCommand(Object.assign({ channel: channel, layer: layer, device: device, transition: transition }, this._createTransitionOptionsObject(transition, transitionDurationOrMaskFile, transitionEasingOrStingDuration, transitionDirectionOrOverlay), { length: length, filter: filter, format: format, channelLayout: channelLayout })));
    }
    /**
     * <https://github.com/CasparCG/help/wiki/AMCP-Protocol#LOADBG>
     */
    loadRouteBg(channel, layer = NaN, route, mode, transition, transitionDurationOrMaskFile, transitionEasingOrStingDuration, transitionDirectionOrOverlay, length, filter, format, channelLayout, auto) {
        return this.do(new AMCP_1.AMCP.LoadRouteBgCommand(Object.assign({ channel, layer, route, mode, transition }, this._createTransitionOptionsObject(transition, transitionDurationOrMaskFile, transitionEasingOrStingDuration, transitionDirectionOrOverlay), { length: length, filter: filter, format: format, channelLayout: channelLayout, auto: auto })));
    }
    /**
     * <https://github.com/CasparCG/help/wiki/AMCP-Protocol#LOADBG>
     */
    loadRouteBgAuto(channel, layer = NaN, route, mode, transition, transitionDurationOrMaskFile, transitionEasingOrStingDuration, transitionDirectionOrOverlay, length, filter, format, channelLayout) {
        return this.do(new AMCP_1.AMCP.LoadRouteBgCommand(Object.assign({ channel, layer, route, mode, transition }, this._createTransitionOptionsObject(transition, transitionDurationOrMaskFile, transitionEasingOrStingDuration, transitionDirectionOrOverlay), { length: length, filter: filter, format: format, channelLayout: channelLayout, auto: true })));
    }
    /**
     * <https://github.com/CasparCG/help/wiki/AMCP-Protocol#LOAD>
     */
    loadRoute(channel, layer = NaN, route, mode, transition, transitionDurationOrMaskFile, transitionEasingOrStingDuration, transitionDirectionOrOverlay, length, filter, format, channelLayout) {
        return this.do(new AMCP_1.AMCP.LoadRouteCommand(Object.assign({ channel, layer, route, mode, transition }, this._createTransitionOptionsObject(transition, transitionDurationOrMaskFile, transitionEasingOrStingDuration, transitionDirectionOrOverlay), { length: length, filter: filter, format: format, channelLayout: channelLayout })));
    }
    /**
     * <https://github.com/CasparCG/help/wiki/AMCP-Protocol#PLAY>
     */
    playRoute(channel, layer = NaN, route, mode, transition, transitionDurationOrMaskFile, transitionEasingOrStingDuration, transitionDirectionOrOverlay, length, filter, format, channelLayout) {
    ...
    loadHtmlPageBg(channel, layer = NaN, clip, transition, transitionDurationOrMaskFile, transitionEasingOrStingDuration, transitionDirectionOrOverlay, auto) {
        return this.do(new AMCP_1.AMCP.LoadHtmlPageBgCommand(Object.assign({ channel: channel, layer: layer, clip: clip, transition: transition }, this._createTransitionOptionsObject(transition, transitionDurationOrMaskFile, transitionEasingOrStingDuration, transitionDirectionOrOverlay), { auto: auto })));
    }
    /**
     *
     */
    loadHtmlPageBgAuto(channel, layer = NaN, url, transition, transitionDurationOrMaskFile, transitionEasingOrStingDuration, transitionDirectionOrOverlay) {
        return this.do(new AMCP_1.AMCP.LoadHtmlPageBgCommand(Object.assign({ channel: channel, layer: layer, url: url, transition: transition }, this._createTransitionOptionsObject(transition, transitionDurationOrMaskFile, transitionEasingOrStingDuration, transitionDirectionOrOverlay), { auto: true })));
    }
    /**
     * <https://github.com/CasparCG/help/wiki/AMCP-Protocol#LOAD>
     */
    loadHtmlPage(channel, layer = NaN, url, transition, transitionDurationOrMaskFile, transitionEasingOrStingDuration, transitionDirectionOrOverlay) {
        return this.do(new AMCP_1.AMCP.LoadHtmlPageCommand(Object.assign({ channel: channel, layer: layer, url: url, transition: transition }, this._createTransitionOptionsObject(transition, transitionDurationOrMaskFile, transitionEasingOrStingDuration, transitionDirectionOrOverlay))));
    }
    playHtmlPage(channel, layer = NaN, url, transition, transitionDurationOrMaskFile, transitionEasingOrStingDuration, transitionDirectionOrOverlay) {
        return this.do(new AMCP_1.AMCP.PlayHtmlPageCommand(Object.assign({ channel: channel, layer: layer, url: url, transition: transition }, this._createTransitionOptionsObject(transition, transitionDurationOrMaskFile, transitionEasingOrStingDuration, transitionDirectionOrOverlay))));
    }
    /**
     ...
     */
    _createTransitionOptionsObject(transition, transitionDurationOrMaskFile, transitionEasingOrStingDuration, transitionDirectionOrOverlay) {
